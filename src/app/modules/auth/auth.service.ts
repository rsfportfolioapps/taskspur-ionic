...
export class AuthService extends BaseService {
  ...
  public forgotPasswordReset(userPasswordReset: UserPasswordReset): Observable<any> {
    return this.postToParams(`/api/account/resetpassword?userId=${userPasswordReset.userId}&newPassword=${userPasswordReset.newPassword}&code=${userPasswordReset.code}`);
  }

  public checkEmail(email: string): Observable<any> {
    return this.postToParams(`/api/account/exists?email=${email}`);
  }
